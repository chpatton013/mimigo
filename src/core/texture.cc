...
static GLvoid LoadTexture(char* image_file, int texID) {
   ...
   cout << "loading Bitmap: " << image_file << endl;
   if (!ImageLoad(image_file, texture_image)) {
      cout << "Image failed to load: " << image_file << " exiting" << endl;
      exit(1);
   }
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, 3,
         texture_image->sizeX, texture_image->sizeY,
         0, GL_RGB, GL_UNSIGNED_BYTE, texture_image->data);
